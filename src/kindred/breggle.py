...
# Trinket table - references item IDs in the catalog
# Format: (roll_min, roll_max, trinket_name, item_id)
BREGGLE_TRINKET_TABLE = AspectTable(
    aspect_type=AspectType.TRINKET,
    die_size=100,
    entries=[
        AspectTableEntry(1, 2, "A bag of divination stones that always answer 'Panic' to any question", item_id="breggle_trinket_01"),
        AspectTableEntry(3, 4, "A bloodstained jester's hat", item_id="breggle_trinket_02"),
        AspectTableEntry(5, 6, "A bloody knife that cannot be cleaned", item_id="breggle_trinket_03"),
        AspectTableEntry(7, 8, "A blue velvet jacket with a hidden pocket which moves when you're not looking", item_id="breggle_trinket_04"),
        AspectTableEntry(9, 10, "A book of poetry that consists primarily of bleating", item_id="breggle_trinket_05"),
        AspectTableEntry(11, 12, "A bottle of noxious perfume that can be smelt up to half a mile away", item_id="breggle_trinket_06"),
        AspectTableEntry(13, 14, "A brass owl statue with eerie black eyes", item_id="breggle_trinket_07"),
        AspectTableEntry(15, 16, "A broken fishing rod that still displays teeth marks from an enormous fish", item_id="breggle_trinket_08"),
        AspectTableEntry(17, 18, "A circular ceramic amulet which displays the current moon phase", item_id="breggle_trinket_09"),
        AspectTableEntry(19, 20, "A clay pot labelled 'Frog Paste,' containing what appears to be frog paste", item_id="breggle_trinket_10"),
        AspectTableEntry(21, 22, "A clump of writhing, black moss that you scraped off a looming monolith one lonely night", item_id="breggle_trinket_11"),
        AspectTableEntry(23, 24, "A collection of papers with scrawled notes detailing your life story, found on a stranger drowned in a ditch", item_id="breggle_trinket_12"),
        AspectTableEntry(25, 26, "A curious mossling wind instrument carved out of a gourd", item_id="breggle_trinket_13"),
        AspectTableEntry(27, 28, "A diorama of two stuffed mice riding stuffed squirrels, jousting", item_id="breggle_trinket_14"),
        AspectTableEntry(29, 30, "A dried mushroom with a face", item_id="breggle_trinket_15"),
        AspectTableEntry(31, 32, "A folio of pressed sprite-wings", item_id="breggle_trinket_16"),
        AspectTableEntry(33, 34, "A gnarled root shaped like a mossling", item_id="breggle_trinket_17"),
        AspectTableEntry(35, 36, "A letter warning that several high-ranked longhorns are secretly crookhorns in disguise", item_id="breggle_trinket_18"),
        AspectTableEntry(37, 38, "A locket with a portrait of a fluffy cat wearing a crown inscribed 'For the love of King Pusskin'", item_id="breggle_trinket_19"),
        AspectTableEntry(39, 40, "A long-nosed masquerade mask", item_id="breggle_trinket_20"),
        AspectTableEntry(41, 42, "A moleskin wristband, anointed with exotic fairy perfume", item_id="breggle_trinket_21"),
        AspectTableEntry(43, 44, "A mossling pipe that blows rainbow-coloured smoke rings", item_id="breggle_trinket_22"),
        AspectTableEntry(45, 46, "A necklace of miscellaneous humanoid teeth", item_id="breggle_trinket_23"),
        AspectTableEntry(47, 48, "A petrified turnip", item_id="breggle_trinket_24"),
        AspectTableEntry(49, 50, "A pig heart that oozes ichor when squeezed", item_id="breggle_trinket_25"),
        AspectTableEntry(51, 52, "A pouch which feels heavy (as if full of pebbles) even when empty", item_id="breggle_trinket_26"),
        AspectTableEntry(53, 54, "A rusty scalpel that once belonged to Lord Malbleat", item_id="breggle_trinket_27"),
        AspectTableEntry(55, 56, "A sack of tasty fried chicken legs", item_id="breggle_trinket_28"),
        AspectTableEntry(57, 58, "A scale said to be from a breggle with a fishtail instead of legs", item_id="breggle_trinket_29"),
        AspectTableEntry(59, 60, "A scroll containing a prophetic warning from an esteemed ancestor, almost indecipherable with age", item_id="breggle_trinket_30"),
        AspectTableEntry(61, 62, "A sheet of parchment with a charcoal sketch of your long lost love", item_id="breggle_trinket_31"),
        AspectTableEntry(63, 64, "A short length of silver cord and a delicate hook, said to catch fairy fish in puddles", item_id="breggle_trinket_32"),
        AspectTableEntry(65, 66, "A shovel stained with the dirt of a thousand graves", item_id="breggle_trinket_33"),
        AspectTableEntry(67, 68, "A stuffed vole dressed in a charming waistcoat", item_id="breggle_trinket_34"),
        AspectTableEntry(69, 70, "A thigh-bone flute", item_id="breggle_trinket_35"),
        AspectTableEntry(71, 72, "A tin whistle whose tones drive cats wild", item_id="breggle_trinket_36"),
        AspectTableEntry(73, 74, "A tiny book of nonsense poetry, bound in purple leather", item_id="breggle_trinket_37"),
        AspectTableEntry(75, 76, "A tiny painting of a four-horned goat", item_id="breggle_trinket_38"),
        AspectTableEntry(77, 78, "A well-loved walking stick with a goat's head handle", item_id="breggle_trinket_39"),
        AspectTableEntry(79, 80, "A wooden Chapes (holy symbol of the Pluritine Church) studded with nails", item_id="breggle_trinket_40"),
        AspectTableEntry(81, 82, "An empty notebook where anything written disappears at sunrise", item_id="breggle_trinket_41"),
        AspectTableEntry(83, 84, "An ornate pie pan, pilfered from a noble's kitchen", item_id="breggle_trinket_42"),
        AspectTableEntry(85, 86, "Black stone dice with white skulls for pips", item_id="breggle_trinket_43"),
        AspectTableEntry(87, 88, "Expensive-looking (but worthless) jewellery, designed for breggle horns", item_id="breggle_trinket_44"),
        AspectTableEntry(89, 90, "String from the bow of a legendary hunter", item_id="breggle_trinket_45"),
        AspectTableEntry(91, 92, "The board pieces for fairy chess, rules unknown", item_id="breggle_trinket_46"),
        AspectTableEntry(93, 94, "The cured skin of a whole deer", item_id="breggle_trinket_47"),
        AspectTableEntry(95, 96, "The horn of an ancestor, hung from a necklace", item_id="breggle_trinket_48"),
        AspectTableEntry(97, 98, "The key to the prison cell you escaped from", item_id="breggle_trinket_49"),
        AspectTableEntry(99, 100, "Your grandmother's creepy glass eye, sometimes you feel her presence watching", item_id="breggle_trinket_50"),
    ]
)
...
BREGGLE_DEFINITION = KindredDefinition(
    ...
    trinket_item_ids=[f"breggle_trinket_{i:02d}" for i in range(1, 51)],
