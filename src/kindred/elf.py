...
ELF_TRINKET_TABLE = AspectTable(
    ...
    die_size=100,
    entries=[
        AspectTableEntry(1, 2, "A bag of caterpillars whose flesh have hallucinogenic properties", item_id="elf_trinket_01"),
        AspectTableEntry(3, 4, "A bag of sticky sweets that never get any smaller when sucked on", item_id="elf_trinket_02"),
        AspectTableEntry(5, 6, "A ball of silvery twine that is invisible in moonlight", item_id="elf_trinket_03"),
        AspectTableEntry(7, 8, "A ball of yarn, gifted to you by a grateful grimalkin", item_id="elf_trinket_04"),
        AspectTableEntry(9, 10, "A black rose that never wilts", item_id="elf_trinket_05"),
        AspectTableEntry(11, 12, "A block of chocolate made with cocoa harvested from a mossling", item_id="elf_trinket_06"),
        AspectTableEntry(13, 14, "A book of amateur poetry, suspected to be by a powerful Fairy noble", item_id="elf_trinket_07"),
        AspectTableEntry(15, 16, "A crown woven from holly and poison ivy", item_id="elf_trinket_08"),
        AspectTableEntry(17, 18, "A daisy that glows in moonlight", item_id="elf_trinket_09"),
        AspectTableEntry(19, 20, "A fancy hat topped with elk antlers", item_id="elf_trinket_10"),
        AspectTableEntry(21, 22, "A fragment of glowing crystal that you found in a dream", item_id="elf_trinket_11"),
        AspectTableEntry(23, 24, "A fragment of horn from an evil unicorn", item_id="elf_trinket_12"),
        AspectTableEntry(25, 26, "A glass bottle that annihilates any liquid poured into it", item_id="elf_trinket_13"),
        AspectTableEntry(27, 28, "A glass jar containing the tiny, frozen form of your only sister", item_id="elf_trinket_14"),
        AspectTableEntry(29, 30, "A glass slipper, stained with blood", item_id="elf_trinket_15"),
        AspectTableEntry(31, 32, "A harp that plays mood-inappropriate music when left unattended", item_id="elf_trinket_16"),
        AspectTableEntry(33, 34, "A Chapes (holy symbol of the Pluritine Church), given by a dying friar decades ago", item_id="elf_trinket_17"),
        AspectTableEntry(35, 36, "A key fashioned from ice that melts in warmth and reforms in cold", item_id="elf_trinket_18"),
        AspectTableEntry(37, 38, "A lantern that burns with a cold, blue flame when lit", item_id="elf_trinket_19"),
        AspectTableEntry(39, 40, "A letter promising your imminent demise, written in High Elfish, delivered over a hundred years ago", item_id="elf_trinket_20"),
        AspectTableEntry(41, 42, "A mortal's heart, freely given", item_id="elf_trinket_21"),
        AspectTableEntry(43, 44, "A mote of sunlight, trapped in a scintillating crystal", item_id="elf_trinket_22"),
        AspectTableEntry(45, 46, "A necklace composed of honeybees", item_id="elf_trinket_23"),
        AspectTableEntry(47, 48, "A nightmare, sealed inside a bottle", item_id="elf_trinket_24"),
        AspectTableEntry(49, 50, "A pan flute stolen from a woodgrue, a single pipe is missing", item_id="elf_trinket_25"),
        AspectTableEntry(51, 52, "A peacock feather whose eye intermittently blinks", item_id="elf_trinket_26"),
        AspectTableEntry(53, 54, "A pleasant dream, distilled into a liquor", item_id="elf_trinket_27"),
        AspectTableEntry(55, 56, "A receipt for a loan of four rare tomes from a Fairy library, books no longer in possession", item_id="elf_trinket_28"),
        AspectTableEntry(57, 58, "A scabbard taken from the fallen body of a great knight", item_id="elf_trinket_29"),
        AspectTableEntry(59, 60, "A sealed scroll allegedly containing one of the Goblin King's myriad names", item_id="elf_trinket_30"),
        AspectTableEntry(61, 62, "A seemingly ordinary acorn that screams when its cap is removed", item_id="elf_trinket_31"),
        AspectTableEntry(63, 64, "A set of horseshoes, designed for a centaur", item_id="elf_trinket_32"),
        AspectTableEntry(65, 66, "A silver spoon that drips honey on command", item_id="elf_trinket_33"),
        AspectTableEntry(67, 68, "A single crow feather, taken from the cloak of the Queen of Blackbirds", item_id="elf_trinket_34"),
        AspectTableEntry(69, 70, "A skeletal finger that writes macabre prophecies at dusk when given means to make marks", item_id="elf_trinket_35"),
        AspectTableEntry(71, 72, "A small bell shaped like a breggle's eye with faint bleating when rung", item_id="elf_trinket_36"),
        AspectTableEntry(73, 74, "A spider that slowly weaves webs in the shape of clothing", item_id="elf_trinket_37"),
        AspectTableEntry(75, 76, "A spyglass that always shows a view of a sea at night", item_id="elf_trinket_38"),
        AspectTableEntry(77, 78, "A thimble that is always magically full of sweet liqueur", item_id="elf_trinket_39"),
        AspectTableEntry(79, 80, "A white-and-gold parasol that creates darkness directly underneath it", item_id="elf_trinket_40"),
        AspectTableEntry(81, 82, "A wolf pelt cloak with the wolf's head still attached, occasionally salivating", item_id="elf_trinket_41"),
        AspectTableEntry(83, 84, "An ancient bronze mask depicting a bearded face", item_id="elf_trinket_42"),
        AspectTableEntry(85, 86, "An empty wine bottle that draws liquid inside until full when held over liquid", item_id="elf_trinket_43"),
        AspectTableEntry(87, 88, "An hourglass which constantly flows in one direction and cannot be inverted", item_id="elf_trinket_44"),
        AspectTableEntry(89, 90, "An icicle that never melts", item_id="elf_trinket_45"),
        AspectTableEntry(91, 92, "Bronze chimes that tinkle in the presence of both ghosts and strong breezes", item_id="elf_trinket_46"),
        AspectTableEntry(93, 94, "Sculpting tools, preternaturally cold to the touch", item_id="elf_trinket_47"),
        AspectTableEntry(95, 96, "Six vials of blood, each drawn from a different Kindred", item_id="elf_trinket_48"),
        AspectTableEntry(97, 98, "Star charts that match no sky seen from Dolmenwood", item_id="elf_trinket_49"),
        AspectTableEntry(99, 100, "The severed tail of a fairy horse", item_id="elf_trinket_50"),
    ]
)
...
ELF_DEFINITION = KindredDefinition(
    ...
    trinket_item_ids=[f"elf_trinket_{i:02d}" for i in range(1, 51)],
