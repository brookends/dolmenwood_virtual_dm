...
class HexCrawlEngine:
    ...
    def attempt_detect_magic_class_ability(
        self,
        hex_id: str,
        character_id: str,
    ) -> dict[str, Any]:
        """
        Attempt to detect magic using Magician/Enchanter class ability.

        Per Dolmenwood rules, Magicians and Enchanters can spend a turn
        attempting to detect magical auras. This requires a skill check.

        Args:
            hex_id: Current hex
            character_id: Character attempting detection (must be Magician/Enchanter)
        ...
        # Verify character is Magician or Enchanter
        valid_classes = ["magician", "enchanter", "magic-user", "mage", "wizard"]
        char_class = character.character_class.lower() if hasattr(character, 'character_class') else ""
        if not any(vc in char_class for vc in valid_classes):
            return {
                "success": False,
                "error": "Only Magicians and Enchanters can use this ability",
            }

        if not self._current_poi:
            return {"success": False, "error": "Must be at a location to detect magic"}
        ...
        # Skill check - INT-based for detecting magical auras
        # DC is typically 10 for obvious magic, higher for concealed magic
        int_mod = character.get_ability_modifier("INT")
        roll = self.dice.roll_d20("detect magic (class ability)")
        total = roll.total + int_mod

        # Add level bonus for higher-level casters
        level = getattr(character, 'level', 1)
        total += level // 2  # +1 per 2 levels

        # Spend a turn
        self.controller.advance_time(turns=1)

        # Track in POI visit
        visit_key = f"{hex_id}:{self._current_poi}"
        if visit_key in self._poi_visits:
            self._poi_visits[visit_key].time_spent_turns += 1

        # Get magical properties
        magical_properties = poi.get_magical_properties()
        if not magical_properties:
            return {
                "success": True,
                "roll": total,
                "detected": [],
                "message": "You sense no magical auras in the immediate area.",
            }

        detected = []
        revealed_count = 0

        for prop in magical_properties:
            # Base DC 10, +5 for hidden magic, -2 per magic school level known
            dc = 10
            ...
            if total >= dc:
                detected.append({
                    ...
                    "description": prop.get("description", "Magical aura detected"),
                    "school": prop.get("school"),
                    "was_hidden": prop.get("hidden", False),
                })

                # Reveal hidden magical locks
                if prop.get("category") == "lock" and prop.get("hidden", False):
                    lock_index = prop.get("index")
                    if lock_index is not None:
                        poi.reveal_magical_lock(lock_index)
                        revealed_count += 1

        if detected:
            message = f"You detect {len(detected)} magical aura(s)."
            if revealed_count > 0:
                message += f" {revealed_count} hidden magical barrier(s) revealed."
            return {
                "success": True,
                "roll": total,
                "detected": detected,
                "revealed_hidden": revealed_count,
                "message": message,
            }
        else:
            return {
                ...
                "roll": total,
                "detected": [],
                "message": "You sense magical energy nearby but cannot pinpoint its source.",
            }

    def cast_detect_magic_spell(
        self,
        hex_id: str,
        character_id: str,
        spell_level: int = 1,
        ...
        """
        Use Detect Magic spell to reveal magical auras.

        Detect Magic spell automatically reveals all magical auras
        within range without requiring a skill check.

        Args:
            hex_id: Current hex
            character_id: Character casting the spell
            spell_level: Level of the detect magic spell (affects range/detail)

        Returns:
            Dictionary with all detected magical properties
        """
        character = self.controller.get_character(character_id)
        if not character:
            return {"success": False, "error": "Character not found"}
        ...
        if not self._current_poi:
            # Can still detect magic at hex level
            hex_data = self._hex_data.get(hex_id)
            if not hex_data:
                return {"success": False, "error": "Hex data not found"}

            # Check hex-level magical effects
            detected = []
            for secret in hex_data.secrets:
                if any(kw in secret.lower() for kw in ["magic", "enchant", "spell", "curse"]):
                    detected.append({
                        "category": "hex_effect",
                        "description": "Magical energy permeates this area",
                    })

            # Check all visible POIs for magical auras
            for poi in hex_data.points_of_interest:
                if poi.is_visible(self._discovered_secrets):
                    magical = poi.get_magical_properties()
                    if magical:
                        detected.append({
                            "category": "location",
                            "poi_name": poi.name,
                            "aura_count": len(magical),
                            "description": f"Magical auras emanate from {poi.name}",
                        })
            ...
            return {
                ...
                "spell_used": True,
                "detected": detected,
                "message": f"The spell reveals {len(detected)} sources of magical energy in the area." if detected else "The spell detects no magical auras nearby.",
            }

        # At a specific POI - get detailed information
        hex_data = self._hex_data.get(hex_id)
        if not hex_data:
            return {"success": False, "error": "Hex data not found"}
        ...
        for prop in magical_properties:
            detail = {
                "category": prop.get("category"),
                "description": prop.get("description", "Magical aura"),
                "was_hidden": prop.get("hidden", False),
            }

            # Higher level spells reveal more detail
            if spell_level >= 2 and prop.get("school"):
                detail["school"] = prop.get("school")

            if spell_level >= 3:
                # Might reveal the specific requirement for magical locks
                if prop.get("category") == "lock":
                    lock_index = prop.get("index")
                    if lock_index is not None and lock_index < len(poi.locks):
                        lock = poi.locks[lock_index]
                        detail["requirement_hint"] = self._get_magical_requirement_hint(lock)
        ...
        # Also detect magic items that might be hidden
        for i, item in enumerate(poi.items):
            if item.get("magical", False) and not item.get("taken", False):
                if item.get("hidden", False) and not item.get("detected", False):
                    item["detected"] = True
                    revealed_count += 1
                detected.append({
                    "category": "item",
                    "name": item.get("name", "Unknown magical item"),
                    "was_hidden": item.get("hidden", False),
                })

        message = f"The spell reveals {len(detected)} magical aura(s)."
        if revealed_count > 0:
            message += f" {revealed_count} previously hidden magical element(s) are now visible."
        ...
        return {
            ...
            "detected": detected,
            "revealed_hidden": revealed_count,
            "message": message if detected else "The spell detects no magical auras at this location.",
        }

    def _get_magical_requirement_hint(self, lock: dict[str, Any]) -> str:
        """Generate a hint about the magical requirement to bypass a lock."""
        requirement = lock.get("requirement", "")
        school = lock.get("magic_school", "")

        if school:
            return f"This barrier responds to {school} magic, specifically {requirement}."
        elif requirement:
            return f"This barrier could be dispelled by {requirement}."
        else:
            return "The nature of this magical barrier is unclear."
